"""Pinned Unicode tables, so what a name is allowed to carry does not move.

Three tables live here, all pinned rather than read from the running
interpreter, and all consulted by :mod:`receipt.corpus`'s one name screen:
:data:`UNASSIGNED_RANGES`, the Unicode 14.0 code points no character had yet
been assigned to; :data:`FORMAT_CONTROL_RANGES`, Unicode 16.0's category
``Cf``; and :data:`DEFAULT_IGNORABLE_RANGES`, Unicode 14.0's
``Default_Ignorable_Code_Point`` property. Each carries its own account
below of where it came from and why it is pinned to the table it is pinned
to. ``receipt.cli`` imports the second of them as well, to escape what a
verdict prints.

The unassigned table, first.

``receipt.corpus`` folds every name it compares — declared paths, pinned
suffixes and roots, and the filesystem entry names that decide closed-world
membership — and a fold key is stable across Unicode tables only for
characters the standard has already encoded. So the module refuses text
carrying an unassigned code point. Consulting the *running* interpreter's
table for that made the refusal a moving target in the other direction:
U+A7CB is unassigned on Python 3.11 through 3.13 (Unicode 14.0 to 15.1) and
assigned on 3.14 (Unicode 16.0), where it folds to U+0264. The same bytes
were therefore refused by one supported interpreter and accepted by the
next, and the acceptance the screen exists to make stable was itself
version-dependent (peer review, round seven).

This module pins the repertoire to Unicode 14.0, the table the oldest
supported interpreter ships, as 698 sorted inclusive ranges of the code
points that table calls ``Cn`` — 829,834 code points in all. A code point
in one of these ranges is refused on every interpreter, whatever the running
table has since assigned to it.

That direction is the safe one, and it is the only one available. Unicode
never unassigns, so this set is a superset of every later table's ``Cn`` set:
pinning it can only refuse text a newer interpreter would have accepted,
never accept text a newer one would refuse. And for everything assigned by
14.0 the stability policies fix case folding and normalization
permanently, so identical text folds identically from 3.11 through 3.14 and
beyond, and is accepted or refused identically with it.

The data is generated by :func:`generate_unassigned_ranges` below, which is
kept here rather than in a build script precisely so it can be re-run: the
test suite calls it and asserts equality with the shipped tuple whenever it
finds itself on an interpreter whose ``unicodedata.unidata_version`` is
``"14.0.0"`` — CI's 3.11 job — and skips elsewhere. Regenerate with::

    python3.11 -c "import receipt._unicode_repertoire as r, pprint;
    pprint.pprint(r.generate_unassigned_ranges())"

Surrogates (``Cs``) and private-use code points (``Co``) are not in this set,
because neither category is ``Cn``. ``receipt.corpus`` refuses lone
surrogates separately, for a different reason.

:data:`FORMAT_CONTROL_RANGES` is Unicode 16.0's category ``Cf``, and it is
pinned in the *opposite* direction for a different job. It bounds what a
producer's text may carry into a rendered verdict, where a code point that
prints as nothing while changing what the reader sees is the hazard, so the
safe direction there is the widest table available rather than the oldest:
the screen refuses a code point in this table *or* one the running
interpreter calls ``Cf``, so a newer table can only widen the set. It moved
here from ``receipt.corpus`` so ``receipt.cli`` can import it too — the
verdict renderer escapes exactly these code points on their way to a
terminal — and its contents are unchanged.

:data:`DEFAULT_IGNORABLE_RANGES` is Unicode 14.0's
``Default_Ignorable_Code_Point`` property, and it exists because a fold key
is not a proof about a target filesystem's own idea of equality. HFS+
ignores default-ignorable code points when it compares names, so
``evil.y\u200dml`` and ``evil.yml`` are one name there and two under any
fold this module can compute. ``receipt.corpus`` refuses a name carrying one
rather than modelling the filesystem; :func:`receipt.corpus._assert_foldable`
says why refusal is the honest answer. Pinned to 14.0 to match the
repertoire above: a code point outside that table is already refused as
unassigned, so the two tables together decide every code point on the same
interpreter-independent footing.

That data is generated by :func:`generate_default_ignorable_ranges`, which
parses the published property file rather than the running interpreter —
...
whose 27 ``Default_Ignorable_Code_Point`` lines merge into the 17 ranges
shipped below, 4,174 code points in all. The test suite carries those 27
lines verbatim and asserts that parsing them reproduces the shipped tuple
exactly, so the generator is checked offline and the source text is on the
record beside it.
"""
...
#: The table this repertoire is pinned to, as the standard names it.
UNICODE_VERSION = "14.0"
#: The same table as ``unicodedata.unidata_version`` spells it, which is what
#: the self-check in the test suite compares against.
UNIDATA_VERSION = "14.0.0"

#: Every code point of general category ``Cn`` in Unicode 14.0, as
#: sorted, non-overlapping, inclusive ``(first, last)`` pairs. 698 ranges.
UNASSIGNED_RANGES: tuple[tuple[int, int], ...] = (
...
#: The first code point of each range, for :func:`is_unassigned`'s bisect.
_RANGE_STARTS: tuple[int, ...] = tuple(first for first, _ in UNASSIGNED_RANGES)


def is_unassigned(code: int) -> bool:
    """Whether this code point was unassigned in Unicode 14.0.

    Bisect rather than a scan: the ranges are sorted and disjoint, so the
    candidate is the last one starting at or below ``code``.
    """

    index = bisect.bisect_right(_RANGE_STARTS, code) - 1
    if index < 0:
        return False
    return code <= UNASSIGNED_RANGES[index][1]
...
#: Every code point with the ``Default_Ignorable_Code_Point`` property in
#: Unicode 14.0, as sorted, non-overlapping, inclusive ``(first, last)``
#: pairs. 17 ranges, 4,174 code points, merged from the 27 lines the
#: published property file carries; see the module docstring for the source
#: URL and :func:`generate_default_ignorable_ranges` for the parser.
#:
#: These are the code points a conforming implementation is invited to
#: ignore, and some filesystems take the invitation when they compare
#: names — HFS+ ignores them, so a name carrying one is the *same* name as
#: the name without it there, while this module's fold key keeps the two
#: distinct. ``receipt.corpus`` refuses rather than modelling that.
...
#: The first code point of each default-ignorable range, for the bisect below.
_IGNORABLE_STARTS: tuple[int, ...] = tuple(
    first for first, _ in DEFAULT_IGNORABLE_RANGES
)


def is_default_ignorable(code: int) -> bool:
    """Whether Unicode 14.0 gave this code point ``Default_Ignorable_Code_Point``.

    Bisect for the same reason :func:`is_unassigned` does: the ranges are
    sorted and disjoint, so the only candidate is the last one starting at or
    below ``code``.
    """

    index = bisect.bisect_right(_IGNORABLE_STARTS, code) - 1
    if index < 0:
        return False
    return code <= DEFAULT_IGNORABLE_RANGES[index][1]


def generate_unassigned_ranges() -> tuple[tuple[int, int], ...]:
    """Recompute :data:`UNASSIGNED_RANGES` from the *running* Unicode table.

    The script that produced the shipped tuple, kept executable so the claim
    the tuple makes can be checked rather than believed. It is only equal to
    the shipped data on an interpreter carrying Unicode 14.0; on a later
    table it returns a strict subset, which is the superset property stated
    in the module docstring, observed from the other side.
    """
...
def generate_default_ignorable_ranges(ucd_text: str) -> tuple[tuple[int, int], ...]:
    """Recompute :data:`DEFAULT_IGNORABLE_RANGES` from a UCD property file.

    ``ucd_text`` is the text of a ``DerivedCoreProperties.txt``; the shipped
    ...
    class, bidi class and a handful of numeric properties, and no query for
    this property at all. So the check the test suite runs is a parse of the
    published text, and it carries those 27 lines verbatim to run it offline.
    """

    ranges: list[tuple[int, int]] = []
    for line in ucd_text.splitlines():
